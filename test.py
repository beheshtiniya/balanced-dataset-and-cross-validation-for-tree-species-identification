...
# مسیر داده‌ها
root_dir = r"E:\FASTRCNN\FASTRCNN\dataset\psudo_labeling\th_0.5_run_5"
combine_result_dir = os.path.join(root_dir, "checkpoints", "COMBINE_RESULT")
...
class trDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        img_path = os.path.join(self.root, r'\FASTRCNN\FASTRCNN\dataset\00_source_code_data_ADDING BEST 0.95 TO TRAIN AND VAL', 'images_rename', self.imgs[idx])
...
test_dataset = trDataset(root_dir, 'val')
...
model.load_state_dict(torch.load(os.path.join(root_dir, r"E:\FASTRCNN\FASTRCNN\dataset\psudo_labeling\th_0.5_run_5\checkpoints", "fasterrcnn_selftrained_final.pth")))
# بارگذاری مدل با حذف پیشوند 'model.' از کلیدها
# model_path = os.path.join(root_dir, "iteration_4", "best_model.pth")
...
test_labels_path = os.path.join(root_dir, "val_labels.csv")
...
# مسیر داده‌ها
root_dir = (r"E:E:\FASTRCNN\FASTRCNN\dataset\psudo_labeling\th_0.5_run_5")
combine_result_dir = os.path.join(root_dir, "checkpoints", "COMBINE_RESULT")
